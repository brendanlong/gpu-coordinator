...
stops it, and the `temp_host` fixture always does.
"""
...
class TempHost:
    ...
    def __init__(
        ...
        root: Path,
        *,
        gpus: Sequence[str] = DEFAULT_GPUS,
        refuse: int = 0,
        ...
        install_fake_nvidia_smi(bin_dir, self.gpus)
    ...
    def config(self) -> dict[str, Any] | None:
        ...
        path = Path(self.home) / "config.json"
        if not path.exists():
            return None
...
@pytest.fixture
def temp_host(tmp_path: Path) -> Iterator[TempHost]:
    host = TempHost(tmp_path / "host")
    try:
        yield host
    finally:
        host.close()
