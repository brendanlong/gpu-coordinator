...
SMI_OUTPUT = """\
GPU-80646905-50a9-afc1-4375-43ca475b15e4, NVIDIA A40, 46068
GPU-83123e65-fe58-7831-6b21-1814b07c25f7, NVIDIA A40, 46068
"""
...
def test_smi_rows_parse_with_and_without_units() -> None:
    ...
    assert parsed[A40] == GpuInfo(name="NVIDIA A40", vram_mib=46068)
...
def test_rows_keep_the_hosts_own_order() -> None:
    info = parse_smi(SMI_OUTPUT)
    assert rows([A40_TWO, A40], info)[0] == ("NVIDIA A40", "45 GB", A40_TWO)
...
def test_host_list_names_the_cards_and_the_driver(control_env: Path, capsys) -> None:
    ...
    assert f"NVIDIA A40                   45 GB   {A40}" in out
...
def test_status_names_the_cards_and_who_holds_them() -> None:
    ...
    assert f"gpu     NVIDIA A40 45 GB  {A40}  busy 20260915-1" in text
    assert f"gpu     NVIDIA A40 45 GB  {A40_TWO}  free" in text
