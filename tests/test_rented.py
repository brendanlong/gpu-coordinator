...
def test_a_record_with_no_offer_or_an_unreadable_one_costs_a_reuse_not_a_crash() -> None:
    ...
    on the path to buying another."""
    assert offer_of(None).name == ""
    assert offer_of({"kind": "runpod", "pod_id": "pod1"}).name == ""
    unreadable = offer_of({"offer": {"vram_gb": "lots"}})
    assert unreadable.name == "" and unreadable.vram_gb == 0
