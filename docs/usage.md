...
**`gpuc cancel <job-id>`** — see [how a job is killed](#how-a-job-is-killed). A
finished job is not touched; its status is the answer, exit 0. That includes a job
whose host is gone, with the status read from the S3 mirror.
...
- **gone**: its rental ended, or its name (from the job index, or a `--host`
  ...
  job the mirror has no final state for, or any job with no `s3_bucket`, went
  with its host: exit 1, saying so.
...
the mirror's final status for a job whose host is `gone`, and null otherwise
or when the mirror has none. `host_state` is the
...
| `cancel` | `{job_id, host, status, source}`: `cancelled` for a queued job, `cancelling` for a running one, a finished job's own status; `source` is `host`, or `mirror` for a job whose host is gone |
