...
ended, or it is no longer registered on this machine) is read from the mirror
at once by `gpuc logs` and `gpuc wait`, and that is the answer, exit 0.
...
requeued_from, submitted_at, s3_prefix, outputs_lost}`. **Requeue one only if
