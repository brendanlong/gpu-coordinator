...
auto_preempt: false                # true: the host stops this job whenever that lets a job
                                   ...
                                   # again. It RE-RUNS FROM THE START, any number of times
...
| `cancel`, `preempt`, `reorder`, `estimate` | `{jobs[], errors[]}`, one entry per id: `{job_id, host, source, error, warnings[]}` plus `status` and the verb's own fields (`priority`; `estimated_runtime_min`; `reorder` adds the queue fields of `submit`). **Check `error`**: when it is set the job was not touched. A `warnings` entry means the mirrored spec kept the old value, so a `requeue` would not carry it. Exit 4 if an id is unknown, else 1 if any `error` |
