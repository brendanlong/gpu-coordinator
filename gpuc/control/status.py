...
class JobView:
    ...
    held_for: list[str] | None = None
    """For a running filler, the jobs holding the cards it is on: it stops as
    soon as one of them can start. None for anything else."""
...
def job_views(payload: dict[str, Any]) -> tuple[list[JobView], list[JobView], list[JobView]]:
    ...
    for entry in payload.get("jobs") or []:
        ...
        view = JobView(
            ...
            held_for=(
                [str(j) for j in entry["held_for"] if isinstance(j, str)]
                if isinstance(entry.get("held_for"), list)
                else None
            ),
...
def _fmt_auto_preempt(job: JobView) -> str:
    ...
    A filler says whose cards those are, since otherwise a job at priority 90
    starting ahead of one at 10, then dying, looks like a bug."""
    if job.held_for:
        return f" auto-preempt, on a card held for {', '.join(job.held_for)}"
