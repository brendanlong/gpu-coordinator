...
def read_api_key(name: str = "runpod") -> str:
    path = paths.secrets_file(name)
    if path.exists():
    ...
    if env_key:
        return env_key
    pod_key = rp_environment().get("RUNPOD_API_KEY", "").strip()
    if pod_key:
        return pod_key
    raise TerminateError(
        f"no provider API key: {path} is missing or empty, RUNPOD_API_KEY is unset, "
        f"and {rp_environment_path()} has no RUNPOD_API_KEY"
    )


def read_pod_id(provider: dict[str, Any] | None = None) -> str:
    env_id = os.environ.get("RUNPOD_POD_ID", "").strip()
    if env_id:
        return env_id
    pod_id = rp_environment().get("RUNPOD_POD_ID", "").strip()
    if pod_id:
        return pod_id
    configured = str((provider or {}).get("pod_id", "")).strip()
    if configured:
        return configured
    raise TerminateError(
        f"no pod id: RUNPOD_POD_ID is unset, {rp_environment_path()} has no "
        "RUNPOD_POD_ID, and config.json has no provider.pod_id"
    )
...
_PROVIDERS: dict[str, TerminateCall] = {"runpod": runpod_terminate}
...
def self_terminate(config: HostConfig, *, terminate_call: TerminateCall | None = None) -> str:
    ...
    call = terminate_call or _PROVIDERS.get(kind)
    if call is None:
